...
const Audit_page = (props) => {
  ...
  const Header = [
    { id: 1, name: listlegveg == "pl_PL" ? "المواقع" : "Locations" },
    {
      id: 2,
      name:
        listlegveg == "pl_PL" ? "وقت بدء التدقيق" : "time of starting audit",
    },
    {
      id: 3,
      name: listlegveg == "pl_PL" ? "مدة التدقيق" : "duration for audit",
    },
    {
      id: 4,
      name:
        listlegveg == "pl_PL" ? "موقع GPS للتدقيق" : "GPS location for audit",
    },
  ...
  return (
    <Grid container>
      <Grid container display={"flex"} className={styles.hadpeg}>
        ...
        >
          <Button
            className={styles.butt_sjjshhsgs}
            onClick={() => {
              Hgfsffsgsgs("", ""), setStartDate("");
              setEndDate("");
          ...
          >
            {listlegveg == "pl_PL" ? "ريسيتوياني" : "Reset"}
          </Button>
          <DatePickerll
            placeholderText={
              listlegveg == "pl_PL" ? "تاريخ البدء" : "Start Date"
            }
          ...
          <DatePickerll
            placeholderText={
              listlegveg == "pl_PL" ? "تاريخ الانتهاء" : "End Date"
            }
            dateFormat="dd MMMM yyyy"
            className={styles.addfast_datepickerll}
            selected={endDate}
          ...
          <FormControl sx={{ m: 1, minWidth: 120 }}>
            <InputLabel id="demo-simple-select-helper-label">
              {listlegveg == "pl_PL" ? "اسم المدقق" : "Auditor Name"}
            </InputLabel>
            <Select
